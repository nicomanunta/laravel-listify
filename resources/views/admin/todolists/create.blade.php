    ...
    <div class="row">
        ...
        <div class="col-12">
            <form action="{{route('admin.todolists.store')}}" method="POST" enctype="multipart/form-data">
                ...
                {{-- TABELLA TASKS --}}
                <div class="form-group mb-3">
                    <label for="tasks[0][description]" class="color-purple font-archivo shadow-purple mb-1">Attività</label>
                    <textarea class="form-control" wrap="soft" name="tasks[0][description]" id="tasks[0][description]" placeholder="Attività">{{ old('description')}}</textarea>
                    @error('description')
                        <div class="text-danger">{{$message}}</div>
                    @enderror
                </div>
