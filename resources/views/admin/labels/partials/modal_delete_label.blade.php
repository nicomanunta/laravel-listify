    ...
    <div class="modal-dialog ">
      <div class="modal-content border-0 bg-modal">
        <div class="modal-header bg-color-yellow border-0">
            <h3 class="modal-title  font-archivo color-grey shadow-purple" id="exampleModalLabel">Conferma di eliminazione </h3>
        ...
        <div class="modal-footer bg-color-yellow border-0">
            <button type="button" class="btn btn-chiudi font-archivo me-1" data-bs-dismiss="modal">Chiudi</button>
                <form action="{{route('admin.labels.destroy', ['label'=> $label->id])}}" method="post">
                @csrf
                @method('DELETE')
                <button type="submit" class="ms-1 btn btn-modal-elimina delete-modale btn-elimina font-archivo">Elimina</button>
            </form>
        </div>
      </div>
    </div>
